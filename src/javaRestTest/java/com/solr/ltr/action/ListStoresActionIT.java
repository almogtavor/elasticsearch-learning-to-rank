...
public class ListStoresActionIT extends BaseIntegrationTest {
    public void testListStore() throws Exception {
        createStore(IndexFeatureStore.indexName("test2"));
        createStore(IndexFeatureStore.indexName("test3"));
        Map<String, ListStoresAction.IndexStoreInfo> infos = new HashMap<>();
        String[] stores = new String[]{IndexFeatureStore.DEFAULT_STORE, IndexFeatureStore.indexName("test2"), IndexFeatureStore.indexName("test3")};
        for (String store : stores) {
            infos.put(IndexFeatureStore.storeName(store),
                    new ListStoresAction.IndexStoreInfo(store, IndexFeatureStore.VERSION, addElements(store)));
        }
        ListStoresAction.ListStoresActionResponse resp = new ListStoresAction.ListStoresActionBuilder(client()).execute().get();
        assertEquals(infos.size(), resp.getStores().size());
        assertEquals(infos.keySet(), resp.getStores().keySet());
        for (String k : infos.keySet()) {
            ListStoresAction.IndexStoreInfo expected = infos.get(k);
            ListStoresAction.IndexStoreInfo actual = resp.getStores().get(k);
