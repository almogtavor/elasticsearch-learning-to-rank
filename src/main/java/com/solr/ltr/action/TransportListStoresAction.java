...
import com.solr.ltr.action.ListStoresAction.ListStoresActionRequest;
import com.solr.ltr.action.ListStoresAction.ListStoresActionResponse;
...
public class TransportListStoresAction extends TransportMasterNodeReadAction<ListStoresActionRequest, ListStoresActionResponse> {
    ...
    protected void masterOperation(ListStoresActionRequest request, ClusterState state,
        ...
        Stream.of(names)
                ...
                .filter((im) -> IndexFeatureStore.STORE_VERSION_PROP.exists(im.getSettings()))
                .forEach((m) -> {
                    req.add(countSearchRequest(m));
                    versions.add(tuple(m.getIndex().getName(), IndexFeatureStore.STORE_VERSION_PROP.get(m.getSettings())));
                });
        if (versions.isEmpty()) {
    ...
    private Tuple<String, Integer> toVersion(String s) {
        ...
        if (index != null && IndexFeatureStore.STORE_VERSION_PROP.exists(index.getSettings())) {
            return new Tuple<>(index.getIndex().getName(), IndexFeatureStore.STORE_VERSION_PROP.get(index.getSettings()));
        }
        return null;
    }
