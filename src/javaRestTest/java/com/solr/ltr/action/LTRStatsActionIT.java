...
public class LTRStatsActionIT extends BaseIntegrationTest {
    private static final String DEFAULT_STORE_NAME = IndexFeatureStore.storeName(IndexFeatureStore.DEFAULT_STORE);

    @SuppressWarnings("unchecked")
    public void testStatsNoStore() throws Exception {
        deleteDefaultStore();
        LTRStatsAction.LTRStatsNodesResponse response = executeRequest();
    ...
    public void testMultipleFeatureStores() throws Exception {
        createStore(IndexFeatureStore.indexName("test1"));
        ...
        assertTrue(stores.containsKey(DEFAULT_STORE_NAME));
        assertTrue(stores.containsKey(IndexFeatureStore.storeName(IndexFeatureStore.indexName("test1"))));
    }

    @SuppressWarnings("unchecked")
    public void testStoreStats() throws Exception {
        String customStoreName = "test";
        String customStore = IndexFeatureStore.indexName("test");
        createStore(customStore);

        Map<String, Map<String, Long>> infos = new HashMap<>();
        infos.put(DEFAULT_STORE_NAME, addElements(IndexFeatureStore.DEFAULT_STORE));
    ...
    private LTRStatsAction.LTRStatsNodesResponse executeRequest() throws ExecutionException, InterruptedException {
        LTRStatsAction.LTRStatsRequestBuilder builder = new LTRStatsAction.LTRStatsRequestBuilder(client());
